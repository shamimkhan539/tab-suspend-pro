...
class OptionsManager {
  ...
  async loadSettings() {
    try {
      ...
      if (result.tabSuspendSettings) {
        this.settings = { ...this.settings, ...result.tabSuspendSettings };
      }
    } catch (error) {
  ...
  async loadTabGroups() {
    try {
      ...
      for (const group of groups) {
        ...
        const isExcluded = this.settings.excludedGroups.includes(group.id);
        
        item.innerHTML = `
          <input type="checkbox" class="group-checkbox" data-group-id="${group.id}" ${isExcluded ? 'checked' : ''}>
  ...
  setupEventListeners() {
    ...
    // Suspend time input
    document.getElementById('suspend-time').addEventListener('input', (e) => {
    ...
    document.getElementById('group-list').addEventListener('change', (e) => {
      if (e.target.classList.contains('group-checkbox')) {
        ...
        if (e.target.checked) {
          ...
          this.settings.excludedGroups = this.settings.excludedGroups.filter(id => id !== groupId);
        }
      }
    });
    ...
    document.getElementById('aggressive-toggle').addEventListener('click', () => {
  ...
  removeWhitelistUrl(url) {
    this.settings.whitelistedUrls = this.settings.whitelistedUrls.filter(u => u !== url);
    this.updateWhitelistUI();
  }
  ...
  updateUI() {
    ...
    // Suspend time
    document.getElementById('suspend-time').value = this.settings.autoSuspendTime;
    ...
    aggressiveToggle.classList.toggle('active', this.settings.aggressiveMode);
  ...
  updateWhitelistUI() {
    ...
    this.settings.whitelistedUrls.forEach(url => {
      ...
      item.innerHTML = `
        <span>${url}</span>
        <button class="btn btn-danger" onclick="optionsManager.removeWhitelistUrl('${url}')">Remove</button>
      `;
