...
class PrivacyDashboard {
    ...
    async loadCloudBackup() {
        try {
            const response = await chrome.runtime.sendMessage({
                action: "cloud-get-backup-status",
            });

            const container = document.getElementById("cloud-backup");
            ...
            if (response && response.success) {
                ...
                container.innerHTML = `
                    ...
                    <div class="metric-row">
                        ...
                        <span class="status-indicator ${
                            ...
                            ${backup.autoSync ? "✓ Enabled" : "✗ Disabled"}
                        </span>
                    </div>
                    <div class="metric-row">
                ...
                container.innerHTML =
                    '<div class="alert alert-warning">Unable to load backup status</div>';
            }
        } catch (error) {
            console.error("Error loading backup status:", error);
            document.getElementById("cloud-backup").innerHTML =
                '<div class="alert alert-danger">Error loading backup status</div>';
        }
    }
